...
def parse_alerts(json_file):
    with open(json_file, 'r') as f:
        data = json.load(f)
    assert 'alerts' in data and isinstance(data['alerts'], list), 'Invalid alert data structure.'
    return data['alerts']
...
def calculate_priority(alert_group):
    ...
    deviation = max((a['value'] - a['threshold']) / a['threshold'] * 100 for a in alert_group)
...
def main():
    ...
    filtered = filter_alerts(alerts, severity='critical', service='payment-processor', minutes=60)
    if not filtered:
